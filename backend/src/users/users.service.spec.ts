...
describe('UsersService', () => {
  ...
  describe('remove', () => {
    ...
    it('should call softDelete on the repository for the given id by an admin', async () => {
      ...
      expect(userRepository.softDelete).toHaveBeenCalledWith(mockStudentUser.id);
    });

    it('should throw NotFoundException if user to delete is not found', async () => {
        mockUserRepository.findOne.mockResolvedValue(null);
        await expect(service.remove(999, mockAdminUser)).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw ForbiddenException if a non-admin tries to delete a user', async () => {
        ...
        await expect(service.remove(mockAdminUser.id, mockStudentUser)).rejects.toThrow(new ForbiddenException('You do not have permission to delete users.'));
    });

    it('should throw ForbiddenException if an admin tries to delete themselves', async () => {
        mockUserRepository.findOne.mockResolvedValue(mockAdminUser);
        await expect(service.remove(mockAdminUser.id, mockAdminUser)).rejects.toThrow(new ForbiddenException('Admins cannot delete their own account.'));
    });
  });
  ...
  describe('update', () => {
    ...
    it('should allow a user to update their own profile', async () => {
        ...
        mockUserRepository.findOne.mockResolvedValue(mockStudentUser);
        mockUserRepository.save.mockResolvedValue({ ...mockStudentUser, ...updateDto });

        const result = await service.update(mockStudentUser.id, updateDto, mockStudentUser);
        expect(userRepository.save).toHaveBeenCalledWith(expect.objectContaining({ fullName: 'New Name' }));
    ...
    it('should prevent a user from updating their own role', async () => {
        ...
        await expect(service.update(mockStudentUser.id, updateDto, mockStudentUser))
            .rejects.toThrow(new ForbiddenException('You cannot change your own role.'));
    });
    ...
    it('should allow an admin to update any user profile', async () => {
        ...
        expect(userRepository.save).toHaveBeenCalledWith(expect.objectContaining({ fullName: 'Updated by Admin' }));
    });
    ...
    it('should prevent a non-admin from updating another user\'s profile', async () => {
        ...
        await expect(service.update(anotherStudent.id, updateDto, mockStudentUser))
            .rejects.toThrow(new ForbiddenException('You do not have permission to update this user.'));
    });
