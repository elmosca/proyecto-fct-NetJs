...
import { Anteproject, AnteprojectTypeEnum, AnteprojectStatusEnum } from './entities/anteproject.entity';
...
describe('AnteprojectsService', () => {
  ...
  const mockAnteprojectRepository = { save: jest.fn(), create: jest.fn(), find: jest.fn(), findOne: jest.fn(), createQueryBuilder: jest.fn(), softDelete: jest.fn() };
  const mockUserRepository = { findOne: jest.fn(), find: jest.fn() };

  beforeEach(async () => {
    mockAnteproject = { id: 1, title: 'Test', tutor: mockTutorUser, students: [mockStudentUser], status: AnteprojectStatusEnum.DRAFT } as Anteproject;

    const module: TestingModule = await Test.createTestingModule({
      providers: [
        AnteprojectsService,
        { provide: getRepositoryToken(Anteproject), useValue: mockAnteprojectRepository },
        { provide: getRepositoryToken(User), useValue: mockUserRepository },
      ],
    }).compile();

    service = module.get<AnteprojectsService>(AnteprojectsService);
    anteprojectRepository = module.get<Repository<Anteproject>>(getRepositoryToken(Anteproject));
  ...
  describe('create', () => {
    ...
    it('should prevent non-students from creating an anteproject', async () => {
        await expect(service.create(createDto, mockTutorUser)).rejects.toThrow(ForbiddenException);
    });
    ...
    it('should prevent a student from creating an anteproject they are not part of', async () => {
        ...
        await expect(service.create(createDto, otherStudent)).rejects.toThrow(ForbiddenException);
    });

    it('should throw NotFoundException if tutor is not found', async () => {
        mockUserRepository.findOne.mockResolvedValue(null);
        await expect(service.create(createDto, mockStudentUser)).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('findAll', () => {
    it('should return all anteprojects for an admin', async () => {
        ...
        expect(anteprojectRepository.find).toHaveBeenCalledWith({ relations: ['tutor', 'students'] });
    });
    ...
    it('should return only own anteprojects for a student', async () => {
        const queryBuilder = { leftJoinAndSelect: jest.fn().mockReturnThis(), where: jest.fn().mockReturnThis(), getMany: jest.fn().mockResolvedValue([mockAnteproject]) };
        mockAnteprojectRepository.createQueryBuilder.mockReturnValue(queryBuilder as any);
        await service.findAll(mockStudentUser);
        expect(anteprojectRepository.createQueryBuilder).toHaveBeenCalledWith('anteproject');
    });
  });
  ...
  describe('findOne', () => {
    ...
    it('should throw ForbiddenException if user cannot access', async () => {
        ...
        mockAnteprojectRepository.findOne.mockResolvedValue(mockAnteproject);
        await expect(service.findOne(mockAnteproject.id, otherUser)).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('update', () => {
    ...
    it('should allow student to update their draft anteproject', async () => {
        mockAnteprojectRepository.findOne.mockResolvedValue(mockAnteproject);
        mockAnteprojectRepository.save.mockResolvedValue({ ...mockAnteproject, ...updateDto });
        await service.update(mockAnteproject.id, updateDto, mockStudentUser);
        expect(anteprojectRepository.save).toHaveBeenCalledWith(expect.objectContaining(updateDto));
    });
    ...
    it('should prevent updating if not in draft status (and user is not admin)', async () => {
        const submittedAnteproject = { ...mockAnteproject, status: AnteprojectStatusEnum.SUBMITTED };
        mockAnteprojectRepository.findOne.mockResolvedValue(submittedAnteproject);
        await expect(service.update(submittedAnteproject.id, updateDto, mockTutorUser)).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('remove', () => {
    it('should allow student to remove their draft anteproject', async () => {
        ...
        expect(anteprojectRepository.softDelete).toHaveBeenCalledWith(mockAnteproject.id);
    });

    it('should prevent removing if not in draft status (and user is not admin)', async () => {
        const submittedAnteproject = { ...mockAnteproject, status: AnteprojectStatusEnum.SUBMITTED };
        mockAnteprojectRepository.findOne.mockResolvedValue(submittedAnteproject);
        await expect(service.remove(submittedAnteproject.id, mockStudentUser)).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('submit', () => {
    it('should allow a student to submit their draft anteproject', async () => {
        ...
        mockAnteprojectRepository.save.mockResolvedValue({ ...anteproject, status: AnteprojectStatusEnum.SUBMITTED });
    ...
    it('should prevent submitting if not in draft status', async () => {
        const submittedAnteproject = { ...mockAnteproject, status: AnteprojectStatusEnum.SUBMITTED };
        mockAnteprojectRepository.findOne.mockResolvedValue(submittedAnteproject);
        await expect(service.submit(submittedAnteproject.id, mockStudentUser)).rejects.toThrow(ForbiddenException);
    });

    it('should prevent a non-student of the project from submitting', async () => {
        const otherStudent = { ...mockStudentUser, id: 99 };
        mockAnteprojectRepository.findOne.mockResolvedValue(mockAnteproject);
        await expect(service.submit(mockAnteproject.id, otherStudent)).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('review', () => {
    it('should allow tutor to put a submitted anteproject under review', async () => {
        const submittedAnteproject = { ...mockAnteproject, status: AnteprojectStatusEnum.SUBMITTED };
        mockAnteprojectRepository.findOne.mockResolvedValue(submittedAnteproject);
        mockAnteprojectRepository.save.mockResolvedValue({ ...submittedAnteproject, status: AnteprojectStatusEnum.UNDER_REVIEW });
        await service.review(submittedAnteproject.id, mockTutorUser);
        expect(anteprojectRepository.save).toHaveBeenCalledWith(expect.objectContaining({ status: AnteprojectStatusEnum.UNDER_REVIEW }));
    });
      
    it('should prevent review if not submitted', async () => {
        const draftAnteproject = { ...mockAnteproject, status: AnteprojectStatusEnum.DRAFT };
        mockAnteprojectRepository.findOne.mockResolvedValue(draftAnteproject);
        await expect(service.review(draftAnteproject.id, mockTutorUser)).rejects.toThrow(ForbiddenException);
    });
      
    it('should prevent student from reviewing', async () => {
        const submittedAnteproject = { ...mockAnteproject, status: AnteprojectStatusEnum.SUBMITTED };
        mockAnteprojectRepository.findOne.mockResolvedValue(submittedAnteproject);
        await expect(service.review(submittedAnteproject.id, mockStudentUser)).rejects.toThrow(ForbiddenException);
    });
  });
    
  describe('approve', () => {
    it('should allow tutor to approve an anteproject under review', async () => {
        const underReviewAnteproject = { ...mockAnteproject, status: AnteprojectStatusEnum.UNDER_REVIEW };
        mockAnteprojectRepository.findOne.mockResolvedValue(underReviewAnteproject);
        await service.approve(underReviewAnteproject.id, mockTutorUser);
        expect(anteprojectRepository.save).toHaveBeenCalledWith(expect.objectContaining({ status: AnteprojectStatusEnum.APPROVED }));
    });
  });

  describe('reject', () => {
    const rejectDto: RejectAnteprojectDto = { tutorComments: 'Not good enough' };
    ...
    it('should allow tutor to reject an anteproject under review', async () => {
        ...
        expect(anteprojectRepository.save).toHaveBeenCalledWith(expect.objectContaining({ 
  ...
  describe('scheduleDefense', () => {
    ...
    it('should allow tutor to schedule defense for an approved anteproject', async () => {
      const approvedAnteproject = { ...mockAnteproject, status: AnteprojectStatusEnum.APPROVED };
      mockAnteprojectRepository.findOne.mockResolvedValue(approvedAnteproject);
      
      await service.scheduleDefense(approvedAnteproject.id, scheduleDto, mockTutorUser);
    ...
    it('should prevent scheduling defense if not approved', async () => {
      const draftAnteproject = { ...mockAnteproject, status: AnteprojectStatusEnum.DRAFT };
      mockAnteprojectRepository.findOne.mockResolvedValue(draftAnteproject);
      await expect(service.scheduleDefense(draftAnteproject.id, scheduleDto, mockTutorUser)).rejects.toThrow(ForbiddenException);
    });

    it('should prevent student from scheduling defense', async () => {
      const approvedAnteproject = { ...mockAnteproject, status: AnteprojectStatusEnum.APPROVED };
      mockAnteprojectRepository.findOne.mockResolvedValue(approvedAnteproject);
      await expect(service.scheduleDefense(approvedAnteproject.id, scheduleDto, mockStudentUser)).rejects.toThrow(ForbiddenException);
    });
  });

  describe('completeDefense', () => {
    it('should allow tutor to complete defense for a scheduled anteproject', async () => {
      const scheduledAnteproject = { ...mockAnteproject, status: AnteprojectStatusEnum.DEFENSE_SCHEDULED };
    ...
    it('should prevent completing defense if not scheduled', async () => {
      const approvedAnteproject = { ...mockAnteproject, status: AnteprojectStatusEnum.APPROVED };
      mockAnteprojectRepository.findOne.mockResolvedValue(approvedAnteproject);
      await expect(service.completeDefense(approvedAnteproject.id, mockTutorUser)).rejects.toThrow(ForbiddenException);
    });

    it('should prevent student from completing defense', async () => {
      const scheduledAnteproject = { ...mockAnteproject, status: AnteprojectStatusEnum.DEFENSE_SCHEDULED };
      mockAnteprojectRepository.findOne.mockResolvedValue(scheduledAnteproject);
      await expect(service.completeDefense(scheduledAnteproject.id, mockStudentUser)).rejects.toThrow(ForbiddenException);
    });
  });
}); 