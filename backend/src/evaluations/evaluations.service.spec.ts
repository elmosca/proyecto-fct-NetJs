...
import { Anteproject, AnteprojectStatusEnum, AnteprojectTypeEnum } from '../anteprojects/entities/anteproject.entity';
...
const mockCriteria: AnteprojectEvaluationCriteria = { id: 1, name: 'Viability', displayOrder: 1, isActive: true, maxScore: 10, createdAt: new Date() };
const mockEvaluation: AnteprojectEvaluation = { id: 1, anteprojectId: mockAnteproject.id, criteriaId: mockCriteria.id, evaluatedById: mockTutorUser.id, score: 8, comments: 'Good', evaluatedAt: new Date(), anteproject: mockAnteproject, criteria: mockCriteria, evaluatedBy: mockTutorUser };
const createDto: CreateEvaluationDto = {
  anteprojectId: mockAnteproject.id,
  evaluations: [
    { criteriaId: mockCriteria.id, score: 8, comments: 'Good' },
  ],
};
...
describe('EvaluationsService', () => {
  ...
  const mockEvaluationsRepository = { save: jest.fn(), find: jest.fn(), create: jest.fn().mockImplementation(dto => dto) };
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        EvaluationsService,
        { provide: getRepositoryToken(AnteprojectEvaluation), useValue: mockEvaluationsRepository },
        { provide: getRepositoryToken(AnteprojectEvaluationCriteria), useValue: mockCriteriaRepository },
        { provide: AnteprojectsService, useValue: mockAnteprojectsService },
      ],
    }).compile();

    service = module.get<EvaluationsService>(EvaluationsService);
    evaluationsRepository = module.get<Repository<AnteprojectEvaluation>>(getRepositoryToken(AnteprojectEvaluation));
    criteriaRepository = module.get<Repository<AnteprojectEvaluationCriteria>>(getRepositoryToken(AnteprojectEvaluationCriteria));
  ...
  describe('createOrUpdate', () => {
    it('should allow a tutor to create evaluations', async () => {
      ...
      expect(anteprojectsService.findOne).toHaveBeenCalledWith(createDto.anteprojectId, mockTutorUser);
      expect(criteriaRepository.find).toHaveBeenCalledWith({ where: { id: In([mockCriteria.id]) } });
      expect(evaluationsRepository.save).toHaveBeenCalled();
    });

    it('should prevent a student from creating evaluations', async () => {
        mockAnteprojectsService.findOne.mockResolvedValue(mockAnteproject);
        await expect(service.createOrUpdate(createDto, mockStudentUser)).rejects.toThrow(ForbiddenException);
    });
    ...
    it('should throw NotFoundException if a criterion does not exist', async () => {
        ...
        await expect(service.createOrUpdate(createDto, mockTutorUser)).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('findAllByAnteproject', () => {
    it('should return evaluations if user has permission', async () => {
        ...
        const result = await service.findAllByAnteproject(mockAnteproject.id, mockTutorUser);
        
        expect(result).toEqual([mockEvaluation]);
        expect(anteprojectsService.findOne).toHaveBeenCalledWith(mockAnteproject.id, mockTutorUser);
    });
    ...
    it('should throw ForbiddenException if findOne throws it', async () => {
        ...
        mockAnteprojectsService.findOne.mockRejectedValue(new ForbiddenException());
        await expect(service.findAllByAnteproject(mockAnteproject.id, mockStudentUser)).rejects.toThrow(ForbiddenException);
    });
  });
}); 