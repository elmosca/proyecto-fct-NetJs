...
const mockProject: Project = { id: 1, title: 'Test Project', description: 'Test Desc', tutor: mockTutorUser, students: [mockStudentUser], status: ProjectStatusEnum.PLANNING, githubMainBranch: 'main', lastActivityAt: new Date(), createdAt: new Date(), updatedAt: new Date(), tutorId: 2 };
...
describe('ProjectsService', () => {
  ...
  describe('create', () => {
    ...
    it('should prevent a student from creating a project', async () => {
        await expect(service.create(createProjectDto, mockStudentUser))
    ...
    it('should prevent a tutor from creating a project for another tutor', async () => {
        ...
        await expect(service.create(dtoForOtherTutor, mockTutorUser))
            .rejects.toThrow(ForbiddenException);
    });
  ...
  describe('findAll', () => {
    it('should return all projects for an admin', async () => {
        ...
        expect(projectRepository.find).toHaveBeenCalledWith({ relations: ['tutor', 'students'] });
    });
    ...
    it('should return only assigned projects for a student', async () => {
        ...
        mockProjectRepository.createQueryBuilder.mockReturnValue(queryBuilder as any);
        ...
        expect(queryBuilder.where).toHaveBeenCalledWith('student.id = :studentId', { studentId: mockStudentUser.id });
    });
  });
  ...
  describe('findOne', () => {
    ...
    it('should throw ForbiddenException if user is not associated with the project', async () => {
        ...
        mockProjectRepository.findOne.mockResolvedValue(mockProject);
        await expect(service.findOne(mockProject.id, anotherTutor)).rejects.toThrow(ForbiddenException);
    });
  ...
  describe('update', () => {
    ...
    it('should allow admin to update a project', async () => {
        ...
        expect(projectRepository.save).toHaveBeenCalledWith(expect.objectContaining(updateDto));
    });
    ...
    it('should allow student to update github url', async () => {
        const githubUpdate: UpdateProjectDto = { githubRepositoryUrl: 'http://new.url' };
        mockProjectRepository.findOne.mockResolvedValue(mockProject);
        mockProjectRepository.save.mockResolvedValue({ ...mockProject, ...githubUpdate });
        await service.update(mockProject.id, githubUpdate, mockStudentUser);
        expect(projectRepository.save).toHaveBeenCalledWith(expect.objectContaining(githubUpdate));
    });

    it('should prevent student from updating other fields', async () => {
        mockProjectRepository.findOne.mockResolvedValue(mockProject);
        await expect(service.update(mockProject.id, updateDto, mockStudentUser)).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('remove', () => {
    ...
    it('should prevent student from deleting a project', async () => {
        mockProjectRepository.findOne.mockResolvedValue(mockProject);
        await expect(service.remove(mockProject.id, mockStudentUser)).rejects.toThrow(ForbiddenException);
    });
  });

  describe('addStudent', () => {
    const newStudent: User = { id: 4, role: RoleEnum.STUDENT, email: 'new@student.com', fullName: 'New Student', passwordHash: '', status: UserStatus.ACTIVE, createdAt: new Date(), updatedAt: new Date() };

    it('should allow project tutor to add a student', async () => {
      mockProjectRepository.findOne.mockResolvedValue(mockProject);
      mockUserRepository.findOne.mockResolvedValue(newStudent);
      mockProjectRepository.save.mockResolvedValue({ ...mockProject, students: [...mockProject.students, newStudent] });

      const result = await service.addStudent(mockProject.id, newStudent.id, mockTutorUser);
    ...
    it('should allow admin to add a student', async () => {
      ...
      const result = await service.addStudent(mockProject.id, newStudent.id, mockAdminUser);
    ...
    it('should prevent another tutor from adding a student', async () => {
      const anotherTutor = { ...mockTutorUser, id: 99 };
      mockProjectRepository.findOne.mockResolvedValue(mockProject);
      await expect(service.addStudent(mockProject.id, newStudent.id, anotherTutor)).rejects.toThrow(ForbiddenException);
    });

    it('should throw NotFoundException if student to add does not exist', async () => {
        mockProjectRepository.findOne.mockResolvedValue(mockProject);
        mockUserRepository.findOne.mockResolvedValue(null);
        await expect(service.addStudent(mockProject.id, 999, mockTutorUser)).rejects.toThrow(NotFoundException);
    });
    ...
    it('should not add a student if they are already in the project', async () => {
        ...
        const result = await service.addStudent(mockProject.id, mockStudentUser.id, mockTutorUser);
  ...
  describe('removeStudent', () => {
    it('should allow project tutor to remove a student', async () => {
        const projectWithStudent = { ...mockProject, students: [mockStudentUser] };
        mockProjectRepository.findOne.mockResolvedValue(projectWithStudent);
        mockProjectRepository.save.mockResolvedValue({ ...mockProject, students: [] });

        const result = await service.removeStudent(mockProject.id, mockStudentUser.id, mockTutorUser);
    ...
    it('should prevent a student from removing another student', async () => {
        mockProjectRepository.findOne.mockResolvedValue(mockProject);
        await expect(service.removeStudent(mockProject.id, mockStudentUser.id, mockStudentUser)).rejects.toThrow(ForbiddenException);
    });
    ...
    it('should not change the project if student to remove is not in it', async () => {
        ...
        const result = await service.removeStudent(mockProject.id, 999, mockTutorUser);
        expect(result.students.length).toBe(mockProject.students.length);
        expect(projectRepository.save).not.toHaveBeenCalled();
    });
  });
