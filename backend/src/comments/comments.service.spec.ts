...
// Mocks
const mockAdminUser: User = { id: 99, role: RoleEnum.ADMIN, email: 'admin@test.com', fullName: 'Admin', passwordHash: '', status: UserStatus.ACTIVE, createdAt: new Date(), updatedAt: new Date() };
...
const mockAuthorUser: User = { id: 1, role: RoleEnum.STUDENT, email: 'test@test.com', fullName: 'Test', passwordHash: '', status: UserStatus.ACTIVE, createdAt: new Date(), updatedAt: new Date() };

const mockProject = { id: 1, tutor: mockTutorUser };
const mockTask = { id: 1, projectId: 1, project: mockProject } as Task;
const mockComment: Comment = { id: 1, content: 'Test comment', taskId: 1, authorId: 1, author: mockAuthorUser, task: mockTask, isInternal: false, createdAt: new Date(), updatedAt: new Date() };
...
describe('CommentsService', () => {
  ...
  const mockCommentRepository = { save: jest.fn(), create: jest.fn(), findOne: jest.fn(), find: jest.fn(), softDelete: jest.fn() };
  ...
  describe('create', () => {
    ...
    it('should create a comment if user has access to the task', async () => {
      ...
      expect(tasksService.findOne).toHaveBeenCalledWith(createDto.taskId, mockAuthorUser);
    ...
    it('should throw error if user cannot access task', async () => {
        ...
        await expect(service.create(createDto, mockAuthorUser)).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('update', () => {
    ...
    it('should allow author to update a comment', async () => {
        ...
        mockCommentRepository.save.mockResolvedValue({ ...mockComment, ...updateDto });

        await service.update(mockComment.id, updateDto, mockAuthorUser);
        expect(commentRepository.save).toHaveBeenCalledWith(expect.objectContaining(updateDto));
    });
    ...
    it('should prevent non-author from updating a comment', async () => {
        ...
        await expect(service.update(mockComment.id, updateDto, otherUser)).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('findAllByTask', () => {
    ...
    it('should throw error if user cannot access task', async () => {
        mockTasksService.findOne.mockRejectedValue(new ForbiddenException());
        await expect(service.findAllByTask(mockTask.id, mockAuthorUser)).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('findOne', () => {
    ...
    it('should throw error if user cannot access task', async () => {
        mockTasksService.findOne.mockRejectedValue(new ForbiddenException());
        mockCommentRepository.findOne.mockResolvedValue(mockComment);
        await expect(service.findOne(mockComment.id, mockAuthorUser)).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('remove', () => {
    ...
    it('should prevent another user from deleting a comment', async () => {
        ...
        mockTasksService.findOne.mockResolvedValue(mockTask);
        mockCommentRepository.findOne.mockResolvedValue(mockComment);
        await expect(service.remove(mockComment.id, anotherUser)).rejects.toThrow(ForbiddenException);
    });
  });
}); 