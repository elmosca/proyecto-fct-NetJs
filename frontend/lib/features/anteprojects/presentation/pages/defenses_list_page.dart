...
import 'package:fct_frontend/core/theme/app_text_styles.dart';
        ...
        children: [
          ...
          DefenseFiltersWidget(
            onFiltersChanged: (newFilters) {
              ref.read(defenseFiltersNotifierProvider.notifier).updateSearch(newFilters.search);
              ref.read(defenseFiltersNotifierProvider.notifier).updateStatus(newFilters.status);
              ref.read(defenseFiltersNotifierProvider.notifier).updateAnteprojectId(newFilters.anteprojectId);
              ref.read(defenseFiltersNotifierProvider.notifier).updateStudentId(newFilters.studentId);
              ref.read(defenseFiltersNotifierProvider.notifier).updateTutorId(newFilters.tutorId);
              ...
              data: (defenses) {
                if (defenses.isEmpty) {
                  return const EmptyStateWidget(
                    ...
                    message: 'No se encontraron defensas con los filtros aplicados.',
                  );
                }
  ...
  void _startDefense(Defense defense) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('¿Estás seguro de que quieres iniciar la defensa de "${defense.anteprojectId}"?'),
        actions: [
          TextButton(
  ...
  void _cancelDefense(Defense defense) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('¿Estás seguro de que quieres cancelar esta defensa?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop(),
            child: const Text('No'),
